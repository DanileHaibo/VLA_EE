...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def inference_ego(
        ...
        **kwargs,
        ...
        output_hidden_states = True  # 需要输出hidden states以支持early exit
        return_dict = self.config.use_return_dict

        # 如果启用early exit，需要获取所有层的hidden states
        if enable_early_exit:
            num_layers = self.config.num_hidden_layers  # 通常是32层
            early_exit_start_layer = 16  # 从第16层开始
            
            # 运行完整模型，获取所有层的hidden states
            outputs = self.model(
                input_ids=inputs,
                attention_mask=attention_mask,
                position_ids=position_ids,
                ...
                inputs_embeds=inputs_embeds,
                use_cache=False,  # early exit不需要cache
                output_attentions=output_attentions,
                output_hidden_states=True,  # 必须为True以获取所有层的输出
            ...
            # 从第16层开始，逐层提取ego feature
            layer_ego_features = []
            if hasattr(outputs, 'hidden_states') and outputs.hidden_states is not None:
                # hidden_states是一个tuple，包含所有层的输出（包括embedding层）
                # hidden_states[0]是embedding输出，hidden_states[1]是第1层的输出，以此类推
                for layer_idx in range(early_exit_start_layer, num_layers):
                    # layer_idx从0开始，所以hidden_states[layer_idx+1]是第layer_idx层的输出
                    current_layer_hidden = outputs.hidden_states[layer_idx + 1]
                    # 提取ego feature
                    selected_hidden_states = current_layer_hidden[loc_positions.to(device=current_layer_hidden.device)]
                    layer_ego_features.append(selected_hidden_states)
            else:
                # 如果没有hidden_states，使用最后一层的输出
                hidden_states = outputs[0]
                selected_hidden_states = hidden_states[loc_positions.to(device=hidden_states.device)]
                layer_ego_features.append(selected_hidden_states)
            
            # 返回所有层的ego features，用于逐层评估
            return layer_ego_features
